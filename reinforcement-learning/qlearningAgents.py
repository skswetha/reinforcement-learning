...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        q_val = 0.0
        for each in self.featExtractor.getFeatures(state, action):
          q_val += self.weights[each] * self.featExtractor.getFeatures(state, action)[each]
        return q_val
        util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        for each in self.featExtractor.getFeatures(state, action):
          self.weights[each] += self.alpha * dif * self.featExtractor.getFeatures(state, action)[each]
